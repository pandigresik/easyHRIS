...
<table class="table table-bordered">
    ...
    <tbody>
        @php
            ...
            $awalBarisKosong = 2;
        @endphp
        @foreach($payrolls->groupBy(function($item){
            \Log::error($item->employee->jobtitle);
            return $item->employee->jobtitle->isContainTitle('security') ? 'Security' : 'Non Security';
        }) as $keyLevel =>  $payrollLevels)
            <tr>
                <td colspan="24">Level Jabatan {{ $jobLevel[$keyLevel] ?? $keyLevel }}</td>
            </tr>
            <tr>
                <td colspan="24"></td>
            </tr>
